...
class Request {
    ...
    sendSMS(from, to, text) {
        this.mocean.sms().create({
            "mocean-from": from,
            "mocean-to": to,
            "mocean-text": text,
            "mocean-resp-format": "JSON",
            "mocean-medium": "NODECLI-SDK"
        }).send((err, res) => {
            if (!err) {
                res = JSON.parse(res);
                if (this.verbose) {
    ...
    getSmsStatus(msgid) {
        ...
        }, {"mocean-resp-format": "JSON", "mocean-msgid": msgid, "mocean-medium": "NODECLI-SDK"})
    }

    verifyRequest(brand, to) {
        this.mocean.verify_request().create({
            "mocean-brand": brand,
            "mocean-to": to,
            "mocean-resp-format": "JSON",
            "mocean-medium": "NODECLI-SDK"
    ...
    verifyValidate(reqid, code) {
        this.mocean.verify_validate().create({
            "mocean-reqid": reqid,
            "mocean-code": code,
            "mocean-resp-format": "JSON",
            "mocean-medium": "NODECLI-SDK"
    ...
    getBalance(callback) {
        ...
        this.mocean.balance().inquiry(callback, {'mocean-resp-format': 'JSON', "mocean-medium": "NODECLI-SDK"});
    }

    getPriceList(params = {}) {
        params["mocean-resp-format"] = "JSON";
        params["mocean-medium"] = "NODECLI-SDK";
